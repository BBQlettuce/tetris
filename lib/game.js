(function () {
  ...
  Game.prototype.prepNextBlock = function() {
    if (this.blockBucket.length <= 0) {
      this.blockBucket = _.shuffle(Game.blocksList);
    }
    var nextBlockType = this.blockBucket.shift();
  ...
  Game.prototype.spawnBlock = function() {
    if (!this.currentBlock || this.currentBlock.stuck) {
      ...
      this.currentBlock.displayBlockAndGhost();
      this.prepNextBlock();
  ...
  Game.prototype.storeBlock = function() {
    ...
    this.currentBlock.undisplayBlockAndGhost();
    if (!this.storedBlock) {
      // this.prepNextBlock();
      this.currentBlock = null;
      this.spawnBlock();
